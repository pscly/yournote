## [2026-03-11 22:56] - 任务：合并远端更新并更新部署（docker compose）
- 原始需求：用户要求“与远端同时合并、更新后重新部署，然后 git push”；并说明远端已更新。
- 实现目标/逻辑：
  - 先 `git fetch` 确认 `main` 与 `origin/main` 同步（避免误以为已更新但实际仍落后）。
  - 使用 `docker compose up -d --build --remove-orphans` 进行更新部署，确保前后端镜像与容器都按最新代码重建/重启。
  - 部署后做最小健康检查：前端首页应返回 200，`/api/access/status` 在未登录状态下返回 401 属于预期。
- 核心变更：
  - 部署：`/root/mnt2/mydocker2/auto_gitlab/younote` 执行 compose 重建并重启容器（`younote-backend-1`、`younote-frontend-1`）。
  - 验证：`http://127.0.0.1:31011/` 返回 200；`http://127.0.0.1:31011/api/access/status` 返回 401。
- 遗留/约束：
  - 本地仍保留同步前备份：`git stash list` 中的 `stash@{0}`（如无需可手动 drop）。

## [2026-03-11 22:23] - 任务：同步仓库到 origin/main 最新
- 原始需求：用户反馈“这个项目似乎更新了”，要求同步更新本地仓库。
- 实现目标/逻辑：
  - 为避免覆盖/丢失本地未提交改动，先通过 `git stash push -u` 备份工作区（包含未跟踪文件）。
  - 从远端 `origin/main` 拉取更新，并使用 fast-forward 保持主线历史干净（`git pull --ff-only`）。
- 核心变更：
  - Git：创建 stash（`wip: sync 前备份 2026-03-11`），当前为 `stash@{0}`。
  - Git：`main` 从 `fc92e81` 快进到 `03b6567`（与 `origin/main` 对齐）。
- 遗留/约束：
  - 如需恢复本地未提交改动：执行 `git stash pop stash@{0}`（可能需要按提示解决冲突）。
  - 如确认不再需要该备份：执行 `git stash drop stash@{0}` 清理。

## [2026-02-23 19:40] - 任务：修复详情页“显示匹配记录”只显示单边
- 原始需求：用户反馈在记录详情页侧栏打开“显示匹配记录”后无反应/只能看到我的或对方的；要求修复并 git push，确认 GitLab CI/CD 部署成功。
- 实现目标/逻辑：
  - 配对关系只选择“包含当前记录作者”的那条，避免错误 fallback 到第一条配对关系导致错配。
  - 合并两边记录列表时按当前 `account_id` 过滤，避免跨账号同用户记录混入导致“看似只显示单边/无变化”。
  - 文案改为“当前用户/配对用户”，与开关语义一致；补充 Playwright 回归用例覆盖开关生效。
- 核心变更：
  - 前端：`frontend/src/pages/DiaryDetail.jsx`
  - E2E：`frontend/tests/diary-detail.spec.js`
- 部署/验证：
  - 本地：`npm run lint`、`npm run build`、`npm run test:e2e -- diary-detail.spec.js` 通过。
  - 推送：外网域名 `u.pscly.cn:33092` SSH 不可达，改用内网 GitLab `192.168.3.5:33092` 推送 main。
  - 部署：`/root/mnt2/mydocker2/auto_gitlab/younote` 已同步到新代码；frontend 容器重启；`http://127.0.0.1:31011/` 返回 200，`/api/access/status` 返回 401。
- 遗留/约束：如需恢复 `u.pscly.cn` 推送，需要排查端口映射/FRP/DDNS（当前以 LAN 地址推送与部署验证为准）。

## [2026-02-23 21:59] - 任务：修复详情页切换日记后“匹配列表”消失
- 原始需求：在对方日记列表开启“显示匹配记录”后，点击自己的某条日记会导致对方日记从侧栏列表消失；再次开启开关也看不到。
- 实现目标/逻辑：
  - 通过 `listSeqRef` 做“最新请求守卫”，避免路由切换（/diary/:id 变化但组件不卸载）时旧请求后返回覆盖新列表（尤其旧 catch 分支清空列表）。
  - 统一 `loadMyDiaries` 在详情页场景下也按当前 `account_id` 过滤，避免单边列表把另一边“挤掉”。
  - 新增 E2E 回归：开启 showMatched 后点击侧栏另一条日记仍应保留双方列表，且加入一次延迟响应模拟竞态。
- 核心变更：
  - `frontend/src/pages/DiaryDetail.jsx`（listSeqRef + account 过滤 + 修复 reloadDiaryList）
  - `frontend/tests/diary-detail.spec.js`（新增“切换日记不丢失配对列表”用例）
- 部署/验证：
  - 本地 E2E：`playwright test diary-detail.spec.js -g "切换日记不应丢失"` 通过。
- 推送：通过 LAN GitLab `ssh://git@192.168.3.5:33092/pscly/younote.git` 推送 main。
- 健康检查：`http://127.0.0.1:31011/` 返回 200；`/api/access/status` 返回 401。

## [2026-02-25 01:48] - 任务：启动新计划 diary-detail-match-bugs（详情页匹配记录翻色/锁定对方）
- 原始需求：修复 `DiaryDetail` 的“显示匹配记录”两类缺陷：从对方日记进入时颜色/归属翻转；以及同账号多条 active 配对关系时切回我的日记会换人/丢对方。
- 实现目标/逻辑：
  - 身份语义固定：`/api/users/paired/{account_id}` 返回的 `user` 永远视为“我(蓝)”，`paired_user` 永远视为“她(红/品红)”，不再随当前打开日记作者翻转。
  - 入口对方优先锁定：从对方日记进入则锁定该对方贯穿 `DiaryDetail` 会话；从我的日记进入且存在多条 active 时默认选择“最新配对”（`paired_time` 最大；缺失则 `rel.id` 最大）。
  - 竞态防护：慢响应不会覆盖已锁定对方（latest-only 写入策略）。
  - 回归：补 Playwright E2E 覆盖“从对方进入不翻色 + 多 active 锁定入口对方 + 从我进入默认最新配对 + 竞态”。
- 核心变更（计划中）：
  - 前端：`frontend/src/pages/DiaryDetail.jsx`
  - E2E：`frontend/tests/diary-detail.spec.js`
  - 证据：`.sisyphus/evidence/`
- 遗留/约束：不做 DB schema 变更；不引入新的全局状态管理库；不改变后端接口返回结构；不重做 UI 布局。

## [2026-02-25 02:17] - 任务：T3 修复 DiaryDetail owner/颜色判定翻转（我蓝她红）
- 原始需求：修复 `DiaryDetail` 的 owner/颜色判定，确保“我蓝她红，不翻转”，尤其从对方日记进入时不把对方误判为 main。
- 实现目标/逻辑：
  - `getDiaryOwner(item)` 仅依据 `pairUsers.main.id` 与 matchedId（优先 `pairUsers.matched.id`，必要时回退 `lockedPartnerUserIdRef.current`）判定 owner，返回严格限定为 `main|matched`。
  - 移除任何 `diary.user_id -> main` 的兜底；并让导出辅助判定与 owner 语义一致，避免 legend 对、边框/导出错。
- 核心变更：`frontend/src/pages/DiaryDetail.jsx`
- 验证：`cd frontend && npm run lint && npm run build` 通过。

## [2026-02-25 03:05] - 任务：T8 E2E：竞态回归（慢接口不覆盖锁定对方）
- 原始需求：在人为延迟 `**/api/users/paired/1` 且 `/api/diaries` 列表接口先返回的情况下，最终稳定态仍为“我=main、她=matched”，且不会出现换人/丢人；测试不依赖 `waitForTimeout`。
- 实现目标/逻辑：
  - `paired` stub 固定延迟 600-1200ms。
  - `diaries` 列表与 `paired` 响应分别记录 fulfill 时间戳，并在断言中强制 `diaries < paired`，确保竞态顺序真实发生。
  - 最终通过等待型断言验证 legend 与 `data-owner` 不翻转、不换人。
- 核心变更：
  - E2E：`frontend/tests/diary-detail.spec.js`（更新“竞态回归”用例，强校验请求返回顺序）
  - 证据：`.sisyphus/evidence/task-t8-e2e-race.txt`
- 验证：`cd frontend && npm run test:e2e -- diary-detail.spec.js -g "竞态"` 通过。

## [2026-02-21 23:13] - 任务：补齐根目录 README 文档（开发/部署/配置/排障）
- 原始需求：用户反馈“项目 README 文档不完善，需要补齐并做得更完整完善”。
- 实现目标/逻辑：
  - 以仓库现有实现为准，补齐“功能概览 / 本地开发 / Docker 部署 / GitLab CI 部署 / 常用配置 / 排错指南 / 自检命令 / 许可证状态”等关键章节。
  - 增加“架构一览”与“页面预览（示意图）”，帮助读者快速理解开发态（Vite proxy）与生产态（Nginx 同源反代）两条链路。
  - 命令块避免臆测：统一引用仓库已有脚本与配置（`run.sh`/`stop.sh`、`docker-compose.yml`、`.env.example`、`frontend/package.json`、后端启动入口等）。
  - 安全约束：强调 `.env` 不提交；并提示 `VITE_*` 会进入前端 bundle 不可放敏感信息；复制 `.env` 使用“存在则不覆盖”的写法避免误覆盖本地配置。
- 核心变更：更新 `README.md`
- 补充资源：新增 `docs/screenshots/*.svg`（页面示意图，可替换为真实截图）
- 遗留/约束：
  - 仓库未提供 `LICENSE` 文件：README 中已提示，如需对外发布建议补齐。
  - 未修改 `backend/README.md` 与 `frontend/README.md`（根 README 已链接到子文档）。

## [2026-02-21 13:13] - 任务：修复 GitLab CI 部署失败并推进生产部署到最新版
- 原始需求：用户要求确认“最新版是否部署成功”，并希望把所有问题一次性修复直到部署成功。
- 实现目标/逻辑：
  - 根因 1：`deploy_u3` 在 `apk add` 阶段偶发拉取 Alpine `APKINDEX` 失败，导致 `bash/curl/rsync/docker-cli-compose` 无法安装 -> job 直接失败。
  - 修复 1：在 `.gitlab-ci.yml` 的 `before_script` 写入多镜像源（官方 + `mirrors.aliyun.com`）并增加重试，提升依赖安装稳定性。
  - 根因 2：容器冷启动/后端就绪存在时延，健康检查偶发拿到非 200/401 的状态码 -> job 误判失败。
  - 修复 2：健康检查增加重试、打印 `http_code`，并在失败时输出 `docker compose ps` + backend/frontend tail logs 便于定位。
- 核心变更：更新 `.gitlab-ci.yml`
- 验证结果：pipeline `1262` 成功；`production` 环境 last_deployment 更新到 `f3b14971...`（deploy job `3216` success）。
- 遗留/约束：无（若后续仍出现依赖安装失败，优先排查 Runner 出站网络/DNS 与镜像源可达性）。
...
## [2026-02-26 14:03] - 任务：/users 空态“没有配对用户” + 已取消配对提示 + E2E
- 原始需求：
  - `/users` 当前配对关系为空时，空态文案改为 `没有配对用户`。
  - `/users` 的“其他主账号（当前未配对）”里：若该账号曾配对过（`/api/users/paired/{accountId}?include_inactive=true` 返回非空）且当前无 active 关系（`is_active` 全为 false），则在账号旁显示黄色文字 `已取消配对`；从未配对（rels=[]）不显示。
  - 补齐 Playwright e2e 覆盖三种状态，并要求测试底座可覆盖 `/api/users/paired/:id` 返回。
- 实现目标/逻辑：
  - 复用 `AllUsers.jsx` 已有的 `userAPI.paired(..., { include_inactive: true })` 结果，不新增请求；通过 `rels.length > 0 && activeRels.length === 0` 推导 `pairingCancelled`。
  - 为测试稳定性新增 `data-testid`：`users-unpaired-main-<accountId>` 与 `users-pairing-cancelled-<accountId>`，避免依赖 Antd DOM 结构。
  - `frontend/tests/testBase.js` 增加 paired mock 覆盖机制（setter + reset），默认仍返回 `[]`，避免影响其它用例。
- 核心变更：
  - 前端：`frontend/src/pages/AllUsers.jsx`
  - E2E 底座：`frontend/tests/testBase.js`
  - E2E 用例：`frontend/tests/users-pairing-status.spec.js`
- 验证：`cd frontend && npm run test:e2e`（41 passed）。
...
## [2026-02-20 23:30] - 任务：交互/流程优化（找记录->阅读）+ 新增“留言记录”页面
- 原始需求：优化项目使用体验（交互/界面 + 功能流程）；并新增一个页面可直接查看“所有有留言的记录”。
- 实现目标/逻辑：
  - 新增独立入口 `/messages`：默认只显示 `msg_count>0` 的记录，便于集中处理“有留言”的记录。
  - 列表->详情“保留上下文”：从列表打开详情时携带来源路径（from）并保存滚动位置；详情页“返回列表”优先回到来源路径，列表页自动恢复滚动位置，减少来回定位成本。
  - 连续阅读：详情页增加“上一条/下一条”，按当前左侧列表顺序快速切换。
- 核心变更：
  - 后端：`backend/app/api/diaries.py`（`GET /api/diaries/query` 新增 `has_msg` 过滤；`order_by` 增加 `msg_count`）；新增 `backend/tests/test_diary_query_has_msg.py`。
  - 前端：`frontend/src/App.jsx`（新增菜单/路由 `/messages`）；新增 `frontend/src/pages/Messages.jsx`；`frontend/src/pages/DiaryList.jsx`（新增“留言”筛选、留言数排序、/messages 默认参数注入、打开详情携带 from + 恢复滚动）；`frontend/src/pages/DiaryDetail.jsx`（返回逻辑优先 from + 上一条/下一条）；`frontend/src/pages/Dashboard.jsx`（留言面板增加“查看所有有留言的记录”快捷入口）。
  - 测试：`frontend/tests/testBase.js`（修复 eslint：去除多余 Boolean / 未使用变量）。
- 验证：
  - 后端：`uv run python -m unittest -v backend/tests/test_diary_query_has_msg.py`、`uv run python -m unittest -v backend/tests/test_postgres_distinct_orderby_regression.py` 通过。
  - 前端：`npm -C frontend run lint`、`npm -C frontend run build` 通过（chunk size warning 为既有提醒）。
- 遗留/约束：未执行 `git commit/push`（需你确认后再提交）。

## [2026-02-25 02:24] - 任务：T1 配对关系选择与“入口对方锁定”状态机
- 原始需求：在 `DiaryDetail` 内集中“配对关系选择”逻辑；会话内锁定入口对方，仅当 `account_id` 变化才重置；并用 latest-only 防止慢响应覆盖锁定；固定 `pairUsers.main=user`、`pairUsers.matched=paired_user` 语义。
- 实现目标/逻辑：
  - 选择入口收敛到 `pickRelationshipForDiaryDetail(...)`：优先复用已锁定对方；否则若入口日记作者命中某条关系的 `paired_user.id` 则选该条；否则选“最新配对”（`paired_time` 最大；缺失则 `rel.id` 最大）。
  - 会话锁定：新增 `lockedAccountIdRef/lockedPartnerUserIdRef`，在 `/diary/:id` 内切换不清空；仅当 `account_id` 变化时清空对方锁定。
  - 竞态防护：新增 `pairSeqRef`，配对关系加载全程以 seq 守卫，旧请求返回不会覆盖已锁定对方/最新选择。
  - 降级：无可用关系时清空 `pairUsers/pairedUserId` 并关闭 `showMatched`。
- 核心变更：`frontend/src/pages/DiaryDetail.jsx`
- 验证：`npm -C frontend run lint`、`npm -C frontend run build` 通过。

## [2026-02-25 03:23:03 CST] - 任务：diary-detail-match-bugs 收尾自检（T7）与竞态证据（T8）补记
- 原始需求：更新会话意图日志（追加记录，不改旧内容）：为本轮 `diary-detail-match-bugs` 的收尾自检(T7)与竞态证据(T8)补齐日志。
- 实现目标/逻辑：
  - 补齐 T7：记录前端完整自检命令与结果，避免后续重构遗漏验收口径。
  - 补齐 T8：将竞态回归的证据文件路径与自检记录关联，便于追溯。
- 核心变更：
  - 会话日志：`.ai_session.md`
  - 会话日志：`frontend/.ai_session.md`
- 验证：
  - 在 `frontend/` 目录执行：`npm run lint`、`npm run build`、`npm run test:e2e` 全部通过；e2e 摘要 `37 passed (1.5m)`。
  - 证据文件（T7）：`.sisyphus/evidence/task-t7-frontend-checks.txt`
  - 证据文件（T8）：`.sisyphus/evidence/task-t8-e2e-race.txt`
- 遗留/约束：无；本条仅做“意图持久化”记录追加，不修改任何业务代码/测试文件。

## [2026-02-25 04:16:40 CST] - 任务：留言记录页默认排序改为入库时间
- 原始需求：留言记录页面（`/messages`）的默认排序改成根据“最后的入库时间”进行排序。
- 实现目标/逻辑：
  - 在 `DiaryList` 的 `/messages` 默认注入逻辑中，将 `orderBy` 从 `msg_count` 改为 `created_at`，并保持 `order=desc`、`hasMsg=1`、`scope=all`。
  - 新增 Playwright E2E：进入 `/messages` 后断言 URL 默认参数与 `GET /api/diaries/query` 请求参数包含 `order_by=created_at&order=desc&has_msg=1&scope=all`。
- 核心变更：
  - 前端：`frontend/src/pages/DiaryList.jsx`
  - E2E：`frontend/tests/messages.spec.js`
- 验证：
  - `npm -C frontend run lint`、`npm -C frontend run build` 通过。
  - `npm -C frontend run test:e2e -- messages.spec.js` 通过。

## [2026-02-26 10:49] - 任务：修复 opencode 启动时报配置校验失败（mcp.exa）
- 原始需求：在项目目录执行 `opencode` 报错：`Configuration is invalid ... ↳ Invalid input mcp.exa`，导致无法启动。
- 实现目标/逻辑：
  - 定位到 `~/.config/opencode/opencode.jsonc` 中 `mcp.exa` 配置。
  - 依据 opencode 配置 schema：`type: "remote"` 的 MCP 配置不支持 `environment` 字段，只支持 `headers/oauth/timeout` 等；因此将鉴权从 `environment.EXA_API_KEY` 改为 `headers.x-api-key`。
- 核心变更：`/root/.config/opencode/opencode.jsonc`（`mcp.exa`：`environment` → `headers`）。
- 验证：再次执行 `opencode` 可正常进入 TUI（不再报配置无效）。
- 遗留/约束：
  - 该修复维持“把 key 写入配置文件”的做法；若后续 opencode 支持环境变量展开/密钥托管，建议将 key 移出配置文件，并确保 `~/.config/opencode/opencode.jsonc` 权限最小化（例如 `chmod 600`）。

## [2026-02-26 16:20] - 任务：修复 test.py 导入副作用（unittest）
- 原始需求：`test.py` 作为脚本在导入时 `raise RuntimeError`，导致 `uv run python -m unittest -v` 在 discovery 阶段因导入 `test` 失败；要求保持 `python test.py` 的请求与写文件行为不变。
- 实现目标/逻辑：
  - 将读取 `NIDERIJI_AUTH`、发起请求、写入 `save_data2.json` 的逻辑封装到 `main()`，仅在 `__name__ == "__main__"` 时执行，避免导入触发副作用。
  - 将 `requests` 延迟导入到 `main()`，降低 `unittest` 仅导入模块时的依赖/副作用风险。
- 核心变更：`test.py`
- 验证：`uv run python -m unittest -v` 不再因导入 `test` 报错（当前仓库未发现 unittest 用例，输出 `Ran 0 tests` 属正常）。
- 遗留/约束：脚本直接执行仍要求设置 `NIDERIJI_AUTH`。

## [2026-02-26 18:10] - 任务：修复后端 unittest discover 显示 Ran 0 tests
- 原始需求：后端全量回归执行 `uv run python -m unittest discover -v` 显示 `Ran 0 tests`；要求让 `backend/tests/test_*.py` 可被 discover，并生成真实回归证据文件。
- 实现目标/逻辑：
  - 根因判断：默认从仓库根目录 discover 时，`backend/` 与 `backend/tests/` 缺少 `__init__.py`，unittest 不会把它们视为 package 递归进入，从而找不到 `backend/tests/test_*.py`。
  - 修复策略：以最小改动补齐包结构（新增 `backend/__init__.py` 与 `backend/tests/__init__.py`），使 discover 能递归进入并以 `backend.tests.*` 模块名导入测试模块。
  - 验收命令：在仓库根目录执行 `uv run python -m unittest discover -s backend/tests -t . -v`，确保输出 `Ran <N> tests`（N>0）且 exit code 为 0；并将完整输出覆盖写入 `.sisyphus/evidence/final-backend-unittest.txt`。
- 核心变更（计划）：`backend/__init__.py`、`backend/tests/__init__.py`、`.sisyphus/evidence/final-backend-unittest.txt`、`.sisyphus/notepads/pairing-cancelled-diaries-paired-empty/learnings.md`。
- 遗留/约束：不改业务逻辑（例如 `backend/app/**`）且不改测试用例内容本身。

## [2026-02-26 18:12] - 任务：后端 unittest discover 回归证据落盘
- 原始需求：生成真实的后端全量回归证据（非 `Ran 0 tests`）。
- 实现目标/逻辑：在补齐 `backend/__init__.py`、`backend/tests/__init__.py` 后，从仓库根目录运行 `uv run python -m unittest discover -s backend/tests -t . -v` 并将完整输出写入证据文件。
- 核心变更：`backend/__init__.py`、`backend/tests/__init__.py`、`.sisyphus/evidence/final-backend-unittest.txt`。
- 验证：`Ran 19 tests`，exit code 0，输出 `OK`；证据文件已覆盖更新。

## [2026-03-04 23:11] - 任务：旧日记详情补全超过次数后停止自动访问
- 原始需求：确认默认多久自动同步一次日记；对超过 3 天前的旧日记，详情补全接口访问 3 次后不再访问。
- 实现目标/逻辑：
  - 默认自动同步间隔保持 20 分钟（`SYNC_INTERVAL_MINUTES` 默认 20）。
  - 新增配置：`DIARY_DETAIL_FETCH_OLD_GIVE_UP_DAYS` / `DIARY_DETAIL_FETCH_OLD_MAX_ATTEMPTS`（默认 3/3）。
  - 在 `CollectorService._save_diaries()` 生成 need_detail_ids 时增加策略：对超过 N 天且 `DiaryDetailFetch.attempts >= M` 的旧日记，跳过 `all_by_ids` 详情补全；手动 refresh 单条日记不受影响。
  - 顺手消除 collector 中 `datetime.utcnow()` 弃用告警：改为 `datetime.now(timezone.utc).replace(tzinfo=None)`，保持“naive 代表 UTC”的既有约定。
- 核心变更：`backend/app/config.py`、`.env.example`、`backend/app/services/collector.py`、`backend/tests/test_detail_fetch_give_up_for_old_diary.py`。
- 验证：`UV_CACHE_DIR=/tmp/uv-cache uv run python -m unittest -v` 通过（Ran 23 tests, OK）。
- 遗留/约束：该限制仅作用于“自动同步中的详情补全”（`all_by_ids`），不影响同步主接口，也不影响手动 refresh。

## [2026-03-07 17:05] - 任务：留言页面增加“只筛选有留言”可选项
- 原始需求：留言页面增加一个可选项，用于只筛选有留言的页面；改完后需要推送 GitHub 和 GitLab。
- 实现目标/逻辑：
  - `/messages` 页面继续默认 `hasMsg=1`，保证默认仍只看有留言。
  - 将现有隐藏强制逻辑改为真正可切换的筛选行为：用户可以在“留言”筛选中切到“全部/仅无留言”，且不会被页面副作用自动改回。
  - `重置全部` 在 `/messages` 下恢复到默认 `hasMsg=1`，排序保持 `created_at desc`，范围保持 `scope=all`。
  - 补充 Playwright 回归测试覆盖默认值、切到“全部”、切到“仅无留言”、重置后恢复默认。
- 核心变更（计划中）：`frontend/src/pages/DiaryList.jsx`、`frontend/tests/messages.spec.js`
- 遗留/约束：不改后端接口与数据库结构；推送时优先现有 GitHub/GitLab 远端，不可达再按仓库既有内网地址回退。

## [2026-03-07 20:10] - 任务：留言页面“仅有留言”改为可切换且可持久化
- 原始需求：留言页面增加“只筛选有留言”的可选项，并在完成后推送 GitHub / GitLab。
- 实现目标/逻辑：
  - `/messages` 首次进入仍默认 `hasMsg=1`，保持默认只看有留言。
  - 当用户明确选择“全部”时，前端把 URL 写成 `hasMsg=all`，这样刷新/分享链接也不会再被默认逻辑强制改回 `hasMsg=1`。
  - `重置条件` 在 `/messages` 下恢复为默认组合：`hasMsg=1`、`orderBy=created_at`、`order=desc`、`scope=all`。
  - 不改后端接口；仍由 `/api/diaries/query` 的 `has_msg` 三态过滤承接查询。
- 核心变更：
  - 前端：`frontend/src/pages/DiaryList.jsx`
  - E2E：`frontend/tests/messages.spec.js`
- 验证：
  - `npm -C frontend run lint` 通过。
  - `npm -C frontend run build` 通过。
  - `npm -C frontend run test:e2e -- messages.spec.js` 通过（4 passed）。
- 遗留/约束：构建仍有 Vite 大包体积 warning，但与本次功能无关，未扩散处理。

## [2026-03-07 20:18] - 任务：留言页面筛选功能交付收口
- 原始需求：完成留言页面筛选调整后提交并推送。
- 实现目标/逻辑：
  - 前端版本由 `0.7.4` 递增到 `0.8.0`，对应本次新增可感知功能。
  - 最终交付状态以当前工作区为准：`/messages` 默认仅有留言；显式 `hasMsg=all`/`hasMsg=0` 可持久化；`重置条件` 恢复默认查询参数。
- 核心变更：`frontend/package.json`、`frontend/package-lock.json`
- 验证：
  - `npm -C frontend run lint && npm -C frontend run build && npm -C frontend run test:e2e -- messages.spec.js` 通过（最终状态）。
- 遗留/约束：仅保留 Vite chunk size warning，未做本次范围外优化。

## [2026-03-07 20:32] - 任务：桌面端左侧导航折叠状态跨刷新保持
- 原始需求：有时候会把左侧栏展开或关闭，希望每次刷新后都能保持上次选择。
- 实现目标/逻辑：
  - 仅持久化桌面端全局导航 `Sider` 的折叠/展开状态。
  - 移动端抽屉 `Drawer` 刷新后仍默认关闭，不做跨刷新记忆。
  - 状态通过前端本地存储保存，首屏直接读取，避免刷新后出现“先展开再收起”的闪动。
- 核心变更（计划中）：`frontend/src/App.jsx`、`frontend/tests/app.spec.js`
- 遗留/约束：不改菜单结构、断点逻辑和其他页面内部侧栏。

## [2026-03-07 20:38] - 任务：桌面端左侧导航折叠状态持久化完成
- 原始需求：刷新页面后保持左侧栏上次的展开/关闭选择。
- 实现目标/逻辑：
  - 新增本地存储键 `yournote_app_sider_collapsed_v1`，只保存桌面端全局导航 `Sider` 的折叠状态。
  - `AppShell` 首屏通过惰性初始化读取本地存储，避免刷新后先展开再收起的闪动。
  - 当 `siderCollapsed` 变化时自动写回本地存储；移动端抽屉 `drawerOpen` 不持久化。
  - 对非法缓存值安全回退为默认展开。
- 核心变更：
  - 前端：`frontend/src/App.jsx`
  - E2E：`frontend/tests/app.spec.js`
  - 版本：`frontend/package.json`、`frontend/package-lock.json`（`0.8.0` -> `0.9.0`）
- 验证：
  - `npm -C frontend run lint && npm -C frontend run build && npm -C frontend run test:e2e -- app.spec.js -g "侧栏折叠状态|本地存储恢复左侧栏|非法值时应回退为默认展开"` 通过。
- 遗留/约束：仅保留 Vite chunk size warning，未做本次范围外优化；移动端抽屉刷新后仍默认关闭。

## [2026-03-11 14:25] - 任务：所有用户页为本地账号用户追加邮箱前缀
- 原始需求：在“所有用户”页面里，把像 `cly（460100）` 这样的本地账号用户展示改成 `cly（460100）(邮箱前缀)`。
- 实现目标/逻辑：
  - 仅改前端 `AllUsers` 页面，不改后端接口。
  - 使用页面现有的 `accountAPI.list()` 数据，按 `nideriji_userid` 把用户与本地账号映射起来。
  - 对所有可映射到本地账号且邮箱非空的用户，在“所有用户”页面内统一追加邮箱前缀展示。
  - 展示规则：邮箱取 `@` 前缀；卡片标题直接拼接为 `姓名（ID）(前缀)`；配对视图/历史提示文案保持同一格式。
- 核心变更（进行中）：
  - 前端：`frontend/src/pages/AllUsers.jsx`
  - E2E：`frontend/tests/users-pairing-status.spec.js`
- 遗留/约束：
  - 范围仅限“所有用户”页面，不扩散到用户详情、仪表盘、账号管理等页面。

## [2026-03-11 14:42] - 任务：所有用户页改为直接显示邮箱
- 原始需求：用户更新要求：`/users` 页面不要再显示用户名，直接显示邮箱；用户名和用户 ID 只在点击进入用户详情页后再看。
- 实现目标/逻辑：
  - `AllUsers` 页面内，凡是能映射到本地账号且有邮箱的用户，统一优先显示完整邮箱。
  - 不再显示这类用户的用户名和 `Nideriji ID` 次级文本；无邮箱映射的用户继续回退为原有标识，避免页面出现无法识别的空标签。
  - 配对视图与“全部用户”卡片保持同一展示口径。
- 核心变更：
  - 前端：`frontend/src/pages/AllUsers.jsx`
  - E2E：`frontend/tests/users-pairing-status.spec.js`
- 版本：`frontend/package.json`、`frontend/package-lock.json`（`0.9.0` -> `0.9.2`）
- 验证：
  - `npm -C frontend run lint` 通过。
  - `npm -C frontend run build` 通过（仍有既有 Vite chunk size warning，无新增构建错误）。
  - `npm -C frontend run test:e2e -- users-pairing-status.spec.js` 通过（4 passed）。
- 遗留/约束：
  - 用户详情页 `UserDetail` 不改，仍保留用户名与用户 ID 展示。

## [2026-03-15 12:30] - 任务：修复站内页面跳转不支持新标签打开 + 发布部署
- 原始需求：网站内很多“进入别的页面”的入口不能用 `Ctrl/⌘ + 鼠标左键` 在新标签打开；要求排查并修复，随后推送到 GitHub + GitLab，并到 `root@192.168.3.5` 更新已部署的老版本。
- 实现目标/逻辑：
  - 根因定位为前端大量入口使用 `onClick + navigate(...)`，缺少真实链接语义，浏览器无法提供中键/修饰键/右键菜单的新标签能力。
  - 统一补一个站内链接封装，优先恢复真实 `href` 与浏览器原生行为；保留“整块可点”的交互，不缩小热区。
  - `DiaryDetail` 的来源页不再只依赖 `location.state.from`，同时通过 URL 参数持久化来源页，保证从新标签打开详情后仍能正确返回原列表与筛选条件。
  - 修复范围覆盖 `App.jsx` 菜单、`DiaryList.jsx`、`DiaryDetail.jsx`、`Dashboard.jsx`、`AllUsers.jsx`、`UserDetail.jsx`、`PairedIncreaseHistory.jsx` 等所有明显属于“跳到别的页面”的入口。
- 核心变更：
  - 前端：新增 `frontend/src/components/AppLink.jsx` 与 `frontend/src/utils/navigation.js`，统一站内链接语义与 `from` 参数拼装/解析。
  - 页面改造：`frontend/src/App.jsx`、`frontend/src/pages/DiaryList.jsx`、`frontend/src/pages/DiaryDetail.jsx`、`frontend/src/pages/Dashboard.jsx`、`frontend/src/pages/AllUsers.jsx`、`frontend/src/pages/UserDetail.jsx`、`frontend/src/pages/PairedIncreaseHistory.jsx`。
  - 测试：`frontend/tests/app.spec.js` 新增“详情入口是真链接且带来源参数”回归。
  - 版本：`frontend/package.json`、`frontend/package-lock.json`（`0.9.2` -> `0.9.3`）。
- 验证：
  - `npm -C frontend run lint` 通过。
  - `npm -C frontend run build` 通过（仅保留既有 Vite chunk size warning）。
  - `npm -C frontend run test:e2e -- app.spec.js -g "应该能够导航到账号管理页面|应该能够查看所有用户|应该能够点击记录查看详情|记录列表详情入口应该是真链接并携带来源参数"` 通过（4 passed）。
  - `npm -C frontend run test:e2e -- users-pairing-status.spec.js` 通过（4 passed）。
  - `npm -C frontend run test:e2e -- diary-detail.spec.js -g "桌面端 - 记录详情页应该显示左侧记录列表|桌面端 - 开启“显示匹配记录”后切换日记不应丢失配对列表"` 通过（2 passed）。
- 遗留/约束：
  - 不改后端接口与数据库。
  - 仅修复页面跳转语义；刷新、同步、筛选、展开等动作仍保持按钮语义。

## [2026-03-15 14:20] - 任务：修复“最近记录”逐字换行并全站排查同类布局问题
- 原始需求：用户发现仪表盘“最近记录”区域文本几乎每个字都换行，要求修复，并检查其他页面是否有同类毛病一起修掉。
- 实现目标/逻辑：
  - 根因初步定位为上次导航语义改造后，`AppLink block` 直接包住 `List.Item.Meta` 等依赖 flex 宽度的结构节点，导致链接元素在列表 flex 布局里宽度收缩，文本被压成极窄列。
  - 先补回归测试，锁住“最近记录链接容器宽度不应异常收缩”。
  - 优先从公共 `AppLink` 的 block 布局行为修复（宽度/最小宽度），让同类页面一并受益；再对高风险页面逐个复核。
  - 同步检查 `Dashboard` 与 `PairedIncreaseHistory` 的列表项链接接入方式，避免只修一处漏掉同类场景。
- 核心变更：
  - 公共组件：`frontend/src/components/AppLink.jsx` 的 `block` 模式补充 `width: 100%`、`minWidth: 0`、`flex: 1 1 auto`，避免在列表 flex 布局里被压窄。
  - 测试：`frontend/tests/app.spec.js` 新增“仪表盘最近记录链接不应窄到导致逐字换行”；`frontend/tests/paired-increase-history.spec.js` 新增“历史记录列表链接不应窄到导致逐字换行”。
  - 版本：`frontend/package.json`、`frontend/package-lock.json`（`0.9.3` -> `0.9.4`）。
- 验证：
  - `npm -C frontend run lint` 通过。
  - `npm -C frontend run build` 通过（仅保留既有 Vite chunk size warning）。
  - `npm -C frontend run test:e2e -- app.spec.js -g "应该能够导航到账号管理页面|应该能够查看所有用户|应该能够点击记录查看详情|记录列表详情入口应该是真链接并携带来源参数|仪表盘最近记录链接不应窄到导致逐字换行"` 通过（5 passed）。
  - `npm -C frontend run test:e2e -- paired-increase-history.spec.js` 通过（2 passed）。
- 遗留/约束：
  - 保留当前新标签打开能力，不回退回纯 `onClick + navigate(...)`。

## [2026-03-15 15:40] - 任务：统一记录相关页面的用户详情点击入口
- 原始需求：用户要求在记录相关页面中，把“当前用户 / 配对用户 / 作者”等明确代表用户的标签改成可点击，并进入对应用户详情页。
- 实现目标/逻辑：
  - 复用现有 `/user/:id` 路由，不新增后端接口。
  - 覆盖 `DiaryDetail` 左侧记录列表顶部 legend、`DiaryList` 作者列，以及 `Dashboard` 的“最近记录 / 新增配对记录”作者标签。
  - 对 `Dashboard` 这类外层已有记录详情入口的区域，拆开“用户详情入口”和“记录详情入口”，避免嵌套链接导致语义错误或点击冲突。
  - 所有新入口补 `white-space: nowrap`，防止作者名和“当前用户 / 配对用户”再次出现逐字换行。
- 核心变更：
  - 前端：`frontend/src/pages/DiaryDetail.jsx`
  - 前端：`frontend/src/pages/DiaryList.jsx`
  - 前端：`frontend/src/pages/Dashboard.jsx`
  - 测试：`frontend/tests/app.spec.js`、`frontend/tests/diary-detail.spec.js`
  - 版本：`frontend/package.json`、`frontend/package-lock.json`（`0.9.4` -> `0.9.5`）。
- 遗留/约束：
  - 完成代码后继续执行：前端校验、git commit、推送 GitHub + GitLab、登录 `root@192.168.3.5` 更新部署并做健康检查。

## [2026-03-15 16:35] - 任务：修复仪表盘“最近记录”与新增配对记录口径不一致
- 原始需求：用户反馈“配对记录数明明增加了，但仪表盘最近记录还是之前的”，怀疑最近改动导致问题。
- 实现目标/逻辑：
  - 先确认根因：不是“作者可点击”改动破坏了接口，而是仪表盘“最近记录”本来按 `ts`（记录最后修改时间）排序；而“配对记录数增加/新增配对记录”按 `created_at`（入库时间）统计，两者口径不同。
  - 保留默认 `ts` 排序，同时在仪表盘新增“按记录时间 / 按入库时间”切换，让用户可以显式查看“最近写了什么”与“最近同步进来了什么”。
  - 后端 `/stats/dashboard` 增加 `latest_order_by` 参数，支持 `ts` 与 `created_at` 两种排序口径，不改响应结构。
- 核心变更：
  - 后端：`backend/app/api/stats.py`
  - 后端测试：`backend/tests/test_postgres_distinct_orderby_regression.py`
  - 前端：`frontend/src/pages/Dashboard.jsx`
  - 前端测试：`frontend/tests/app.spec.js`
  - 版本：`frontend/package.json`、`frontend/package-lock.json`（`0.9.5` -> `0.9.6`）。
- 验证：
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python -m unittest backend.tests.test_postgres_distinct_orderby_regression` 通过。
  - `npm -C frontend run lint` 通过。
  - `npm -C frontend run build` 通过（仅保留既有 Vite chunk size warning）。
  - `env FRONTEND_PORT=4173 npm -C frontend run test:e2e -- app.spec.js -g "仪表盘最近记录链接不应窄到导致逐字换行|仪表盘最近记录作者标签应跳转到用户详情而不是记录详情|仪表盘最近记录应支持在按记录时间和按入库时间之间切换，并记住选择"` 通过（3 passed）。
- 遗留/约束：
  - 默认排序仍保持 `ts`，避免直接改变老用户原有认知；切换值通过 localStorage 持久化。
