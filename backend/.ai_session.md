...
## [2026-02-26 14:05] - 任务：sync 基于 diaries_paired 失活/激活配对关系
- 原始需求：当上游 `/api/v2/sync/` 返回 `diaries_paired` 为空（缺失/None/[]）时，视为取消配对并把该 `account_id` 下所有 active 关系置 inactive；当 `diaries_paired` 非空时，确保“当前配对关系”为 active，并保证同一账号最多 1 条 active（其余失活），不删除任何历史记录。
- 实现目标/逻辑：
  - 在 `CollectorService.sync_account()` 以 `paired_diaries = rdata.get('diaries_paired', None)` 为准：仅 `list 且 len>0` 才算仍配对；否则执行关系失活并跳过 `_save_paired_user_info()`。
  - 在 `_save_paired_user_info()` 内支持：已存在关系则更新 `is_active=True`（可从历史 inactive 重新激活），上游提供 `paired_time` 则写入；并通过一次 UPDATE 失活该账号下其它 active，保证单 active。
- 核心变更：
  - `backend/app/services/collector.py`
  - `backend/tests/test_paired_relationship_state.py`
- 验证：`uv run python -m unittest -v backend.tests.test_paired_relationship_state` 通过。

## [2026-02-26 14:18] - 任务：补齐配对取消逻辑的可观测性（warning 日志）
- 原始需求：当 `diaries_paired` 缺失/None 或类型异常（非 list）时，仍按取消配对处理，但需要在每次 sync 记录一次 `logger.warning`（包含 `account_id`，类型异常还需包含类型）。
- 核心变更：
  - `backend/app/services/collector.py`：在 `paired_diaries = rdata.get('diaries_paired', None)` 旁新增缺失/None/非 list 的 warning 日志。
  - `backend/tests/test_paired_relationship_state.py`：新增 `diaries_paired` 非 list（空字符串）用例，确保也会失活关系。
- 验证：`uv run python -m unittest -v backend.tests.test_paired_relationship_state` 通过。

## [2026-03-04 23:11] - 任务：旧日记详情补全超过次数后停止自动访问
- 原始需求：对超过 3 天前的旧日记，详情补全（`all_by_ids`）访问 3 次后就不再自动访问；同时确认默认定时同步频率。
- 实现目标/逻辑：
  - 默认定时同步间隔保持 20 分钟（`SYNC_INTERVAL_MINUTES` 默认 20，不改现有调度器行为）。
  - 新增配置：`DIARY_DETAIL_FETCH_OLD_GIVE_UP_DAYS` / `DIARY_DETAIL_FETCH_OLD_MAX_ATTEMPTS`（默认 3/3；若任意值 <=0 则禁用该限制）。
  - 在 `CollectorService._save_diaries()` 计算 need_detail_ids 时新增判断：若日记创建时间距今 >N 天且 `DiaryDetailFetch.attempts >= M`，则跳过详情补全；避免旧日记长期失败导致无限重试。
  - 该限制不影响 `CollectorService.refresh_diary()`（手动刷新仍会尝试拉取详情）。
- 核心变更：`backend/app/config.py`、`backend/app/services/collector.py`、`backend/tests/test_detail_fetch_give_up_for_old_diary.py`、`.env.example`。
- 验证：`UV_CACHE_DIR=/tmp/uv-cache uv run python -m unittest -v` 通过（全量 23 tests OK）。
