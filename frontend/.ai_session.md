...
## [2026-02-20 15:46] - 任务：新增“书签/收藏”入口（路由 + 菜单高亮）
- 原始需求：前端增加 `/bookmarks` 页面入口；菜单新增“书签/收藏”项；当 pathname 以 `/bookmarks` 开头时菜单高亮选中；路由新增 `/bookmarks` 渲染页面。
- 实现目标/逻辑：
  - 在 `App.jsx` 增加 navItems 菜单项（key 固定为 `/bookmarks`），并在 `selectedKey` 规则中对 `/bookmarks` 前缀做归一化。
  - 新增 `Bookmarks` 页面做最薄封装，直接复用 `DiaryList`（后续 T8 再做筛选/排序等行为）。
- 核心变更：
  - `frontend/src/App.jsx`：新增菜单项、selectedKey 规则与 `/bookmarks/*` 路由。
  - `frontend/src/pages/Bookmarks.jsx`：新增页面组件。
- 遗留/约束：不改 `DiaryList`；不引入新依赖；`npm run build` 通过（chunk size warning 仍为既有提示）。

## [2026-02-20 16:45] - 任务：新增书签关键链路 E2E（列表/详情/书签页）
- 原始需求：新增 Playwright E2E 用例覆盖“列表/详情/书签页”关键链路（含导出下载内容断言）。
- 实现目标/逻辑：
  - 在 `/diaries?view=list` 对样例记录执行「加入书签」，断言按钮 title 从「加入书签」切换为「取消书签」。
  - 从菜单进入 `/bookmarks` 后强制 `view=list`，保证出现 Table + rowSelection + 顶部批量按钮。
  - 在书签页勾选当前页后：导出 Markdown 通过 `download` 事件保存并读取文件，断言内容包含 `/diary/`、标题与日期；随后执行「批量取消书签」并断言 emptyText 变为「暂无记录」。
  - 在 `/diary/1` 覆盖「收藏/取消收藏」按钮文案切换，不依赖初始状态。
- 核心变更：`frontend/tests/diary-bookmarks.spec.js`。
- 遗留/约束：不修改 `frontend/tests/testBase.js` 与 `frontend/src/**`；不引入新依赖；选择器优先使用 role/title/按钮文案。

## [2026-02-20 23:30] - 任务：新增“留言记录”页面 + 列表->详情上下文返回
- 原始需求：新增一个页面可直接查看“所有有留言的记录”；并继续优化“找记录->阅读详情”的流程。
- 实现目标/逻辑：
  - 新增路由 `/messages`：默认注入 `hasMsg=1`、`orderBy=msg_count`、`scope=all`，进入即展示所有 `msg_count>0` 的记录。
  - `DiaryList` 增强：新增“留言”筛选（全部/仅有/仅无）；排序增加留言数 asc/desc；从列表打开详情时携带 `state.from` 并保存滚动位置，返回后恢复滚动。
  - `DiaryDetail` 增强：返回按钮优先回到 `from`；新增“上一条/下一条”按钮便于连续阅读。
  - 仪表盘：留言面板新增入口直达 `/messages`。
- 核心变更：
  - `frontend/src/App.jsx`
  - `frontend/src/pages/Messages.jsx`
  - `frontend/src/pages/DiaryList.jsx`
  - `frontend/src/pages/DiaryDetail.jsx`
  - `frontend/src/pages/Dashboard.jsx`
- 验证：`npm -C frontend run lint`、`npm -C frontend run build` 通过。
- 遗留/约束：不自动执行 `git commit/push`。

## [2026-02-25 02:17] - 任务：DiaryDetail owner/颜色判定修复（我蓝她红）
- 原始需求：从对方日记进入详情页时，列表项 owner/颜色（边框/背景）不应翻转。
- 实现目标/逻辑：`getDiaryOwner` 只依据 `pairUsers.main.id` 与 matchedId（优先 `pairUsers.matched.id`，必要时回退 `lockedPartnerUserIdRef.current`）判定，并移除 `diary.user_id` 的 main 兜底。
- 核心变更：`frontend/src/pages/DiaryDetail.jsx`
- 验证：`npm -C frontend run lint`、`npm -C frontend run build` 通过。

## [2026-02-25 03:03] - 任务：T4 E2E 从对方日记进入时 legend/归属不翻转
- 原始需求：新增 Playwright 用例，从 `/diary/2`（对方日记）进入后打开“显示匹配记录”，断言 legend 与列表项 `data-owner` 不翻转。
- 实现目标/逻辑：
  - stub `/api/users/paired/1`：`user(id=1,name=用户1)` 作为 main/当前用户，`paired_user(id=2,name=用户2)` 作为 matched/配对用户。
  - stub `/api/diaries/:id`：diary 1 属于 user 1，diary 2 属于 user 2；stub `/api/diaries?user_id=...` 确保两边各有 1 条记录。
  - 断言：`diary-owner-legend-main|matched` 同时包含身份文案 + 对应用户名；`diary-sider-item-1|2` 的 `data-owner` 分别为 `main|matched`。
- 核心变更：`frontend/tests/diary-detail.spec.js`（新增“从对方日记进入时 legend/归属不翻转”用例）。
- 验证：`cd frontend && npm run test:e2e -- diary-detail.spec.js -g "从对方"` 通过。

## [2026-02-25 03:55] - 任务：T8 E2E 竞态回归（慢接口不覆盖锁定对方）
- 原始需求：新增 1 个 Playwright 用例从 `/diary/2` 进入；人为延迟 `GET /api/users/paired/1` 600-1200ms 后再 fulfill，且 `GET /api/diaries*` 先返回并渲染至少 1 个列表项；最终稳定态下开启“显示匹配记录”后 legend main=用户1、matched=用户2，且 `diary-sider-item-1|2` 的 `data-owner` 分别为 `main|matched`（不换人/不丢人）。
- 实现目标/逻辑：在用例内 stub `/api/users/paired/1` 与 `/api/diaries/*`、`/api/diaries**`；用 Promise marker + route 内延迟确保“列表先到、配对后到”的竞态；等待稳定态使用 `expect(locator).toBeEnabled()/toBeVisible()/toHaveAttribute()`，不以 `waitForTimeout` 作为断言手段。
- 核心变更：`frontend/tests/diary-detail.spec.js`（新增“竞态：paired 接口慢于 diaries 列表时不覆盖入口锁定对方”用例）。
...
- 实现目标/逻辑：
  - T7：记录前端完整自检的命令与结果（lint/build/e2e 全绿），作为本轮验收基线。
  - T8：补充竞态回归证据文件路径，便于后续定位“列表先到、配对后到”的顺序断言依据。
- 核心变更：
...
- 实现目标/逻辑：
  - 修改 `/messages` 的默认注入参数：`orderBy=created_at`、`order=desc`（并保持 `hasMsg=1`、`scope=all`）。
  - 新增 Playwright E2E：进入 `/messages` 后断言 URL 默认参数与 `GET /api/diaries/query` 请求参数一致（`order_by=created_at&order=desc&has_msg=1&scope=all`）。
- 核心变更：
  - `frontend/src/pages/DiaryList.jsx`
  - `frontend/tests/messages.spec.js`
- 验证：
